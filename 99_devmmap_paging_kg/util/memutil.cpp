...
int main(int argc, char *argv[])
{
	...
	for (int i = 0; i < op_cnt; i++) {
		...
		case OP_MAPREAD:{
				...
				for (int i = 0; i < len; i++) {
					if (msg_len == 0)	//if no message is given just read everything
					{
						c = *(device_mem + i);
					} else if (*(device_mem + i) !=
						   msg[i % msg_len]) {
						//if data read does not match with msg then failure
						ret = EXIT_FAILURE;
					}
				}
		...
		case OP_MAPWRITE:{
				...
				// write msg repeatedly for whole 1MB
				for (int j = 0; j < len; j += msg_len) {
					for (i = j; i < j + msg_len && i < len;
					     i++) {
						*(device_mem + i) =
						    msg[i % msg_len];
						//cout<<*(device_mem+i);
